...
function listar_en_compras() {
  tabla_en_compras = $("#lista_productos_data")
    .dataTable({
      aProcessing: true, //Activamos el procesamiento del datatables
      aServerSide: true, //Paginación y filtrado realizados por el servidor
      dom: "Bfrtip", //Definimos los elementos del control de tabla
...
function listarDetalles() {
  ...
  for (var i = 0; i < detalles.length; i++) {
    if (detalles[i].estado == 1) {
      var importe = (detalles[i].importe =
        detalles[i].cantidad * detalles[i].precio);

      importe = detalles[i].importe =
        detalles[i].importe - (detalles[i].importe * detalles[i].dscto) / 100;
      var filas =
        filas +
        "<tr><td>" +
        (i + 1) +
        "</td> <td name='producto[]'>" +
        detalles[i].producto +
        "</td> <td name='precio[]' id='precio[]'>" +
        detalles[i].moneda +
        " " +
        detalles[i].precio +
        "</td> <td>" +
        detalles[i].stock +
        "</td> <td><input type='number' class='cantidad input-group-sm' name='cantidad[]' id='cantidad[]' onClick='setCantidad(event, this, " +
        i +
        ");' onKeyUp='setCantidad(event, this, " +
        i +
        ");' value='" +
        detalles[i].cantidad +
        "'></td>  <td><input type='number' name='descuento[]' id='descuento[]' onClick='setDescuento(event, this, " +
        i +
        ");' onKeyUp='setDescuento(event, this, " +
        i +
        ");' value='" +
        detalles[i].dscto +
        "'></td> <td> <span name='importe[]' id='importe" +
        i +
        "'>" +
        detalles[i].moneda +
        " " +
        detalles[i].importe +
        "</span> </td> <td>  <button href='#' class='btn btn-danger btn-lg' role='button' onClick='eliminarProdVenta(event, " +
        i +
...
Así que solo agregué esa resta a la operación*/
  ...
  var importe = (detalles[idx].importe =
    detalles[idx].cantidad * detalles[idx].precio);
  importe = detalles[idx].importe =
    detalles[idx].importe - (detalles[idx].importe * detalles[idx].dscto) / 100;
...
function eliminarProdVenta(event, idx) {
  event.preventDefault();
...
function registrarCompra() {
  ...
  var validarNegativo = total.split(" ");
  if (parseInt(validarNegativo[1]) <= 0) {
    return false;
  }
  if (
    cedula != "" &&
    ...
    $.ajax({
      ...
      method: "POST",
      data: {
        arrayCompra: JSON.stringify(detalles),
      ...
      success: function (data) {
        ...
        setTimeout(
          "bootbox.alert('Se ha registrado la compra con éxito');",
          100
        );
    ...
    bootbox.alert(
      "Debe agregar un producto, los campos del proveedor y el tipo de pago"
    );
    return false;
  }
}
...
function listar_en_ventas() {
  tabla_en_ventas = $("#lista_productos_ventas_data")
    .dataTable({
      aProcessing: true, //Activamos el procesamiento del datatables
      aServerSide: true, //Paginación y filtrado realizados por el servidor
      dom: "Bfrtip", //Definimos los elementos del control de tabla
  ...
  for (var i = 0; i < detalles.length; i++) {
    if (detalles[i].estado == 1) {
      ...
      var filas =
        ...
        "</td> <td>" +
        detalles[i].stock +
        "</td> <td> <input type='text' class='cantidad' name='cantidad[]' id=cantidad_" +
        i +
        " onClick='setCantidad(event, this, " +
        i +
        ");' onKeyUp='setCantidadAjax(event, this, " +
        i +
        ");' value='" +
        detalles[i].cantidad +
        "'> </td>  <td><input type='text' name='descuento[]' id='descuento[]' onClick='setDescuento(event, this, " +
        i +
        ");' onKeyUp='setDescuento(event, this, " +
        i +
        ");' value='" +
        ...
        " " +
        detalles[i].importe +
        "</span> </td> <td>  <button href='#' class='btn btn-danger btn-lg' role='button' onClick='eliminarProd(event, " +
        i +
        ");' aria-pressed='true'><span class='glyphicon glyphicon-trash'></span> </button></td>   </tr>";
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (
        ...
        $(".btn_producto").attr({
          "data-target": "#lista_productos_ventas_Modal",
        });
...
function registrarVenta() {
  ...
  if (
    ...
    $.ajax({
      ...
      method: "POST",
      data: {
        arrayVenta: JSON.stringify(detalles),
      ...
      success: function (data) {
        ...
        //muestra un mensaje de exito
        setTimeout(
          "bootbox.alert('Se ha registrado la venta con éxito');",
          100
        );
    ...
    bootbox.alert(
      "Debe agregar un producto, los campos del cliente y el tipo de pago"
    );
    return false;
  }
}
